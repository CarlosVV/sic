...
SET @Query1 =  ('
					SELECT @Count = SUM(CASE	WHEN '+@ColName1+' IS NULL THEN 1 
												ELSE CASE 
															WHEN ISNUMERIC('+@ColName1+') = 1 THEN 1 
															WHEN LEN(LTRIM(RTRIM('+@ColName1+')))>1 THEN 1 
															ELSE 0 
													END
										END)
...
SET @Query2 =  ('
					SELECT @Count = SUM(CASE	WHEN '+@ColName2+' IS NULL THEN 1 
												ELSE CASE 
															WHEN ISNUMERIC('+@ColName2+') = 1 THEN 1 
															WHEN LEN(LTRIM(RTRIM('+@ColName2+')))>1 THEN 1 
															ELSE 0 
													END
										END)
...
SET @Query1 = '
		SELECT	''Testing Count of NULL Records: ['+@DBName1+'].['+@SchemaName1+'].['+@TableName1+'].['+@ColName1+'] vs ['+@DBName2+'].['+@SchemaName2+'].['+@TableName2+'].['+@ColName2+']'' AS Unit_Test_Name
				, '''+ @Result								+'''	AS Result
